...
void TFT_Display::SetForegroundColor(uint32_t const color) // 24 bit color, initialized in BLACK
{
	foregroundColor = color;
}

void TFT_Display::SetBackgroundColor(uint32_t const color) // 24 bit color, initialized in WHITE
{
	backgroundColor = color;
}

void TFT_Display::SetFont(FontType const * const font)
{
	this->font = font;
}

void TFT_Display::ClearScreen(void)
{
	RectType const rect = {	{ 0, 0 }, TFT_Display::WIDTH, TFT_Display::HEIGHT };
...
void TFT_Display::DrawChar(PositionType const * const p, char const c)
{
	...
	/* Check boundaries */
	if (((p->x + glyph->width) > TFT_Display::WIDTH) || ((p->y + font->height) > TFT_Display::HEIGHT))
	{
		return;
	}
...
{
	uint16_t const xe = rect->pos.x + rect->width - 1;
	uint16_t const ye = rect->pos.y + rect->height - 1;
...
uint32_t TFT_Display::ToBSRRValue(uint8_t const data)
{
	...
	uint32_t invData = ~data;

	/* Add '0' values of data to BSRR write value and return result */
	return bsrrVal | (invData << TFT_ShiftHalfWord);
}

uint16_t TFT_Display::To16bpp(uint32_t const rgbPixelValue)
{
	static uint32_t const maskRed = 0x00F80000;
	static uint32_t const maskGreen = 0x0000FC00;
	static uint32_t const maskBlue = 0x000000F8;
	static uint8_t const shiftRed = 8;
	static uint8_t const shiftGreen = 5;
	static uint8_t const shiftBlue = 3;
