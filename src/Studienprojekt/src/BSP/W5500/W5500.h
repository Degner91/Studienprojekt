...
#ifndef BSP_W5500_W5500_H_
#define BSP_W5500_W5500_H_
...
private:
	...
	static const uint8_t AccessModeWrite = (0x01 << 2); //< SPI interface Read operation in Control Phase
	static const uint8_t BlockSelectCReg = (0x00 << 3); //< Common register block in Control Phase
	static const uint8_t BlockSelectSReg = (0x01 << 3); //< Socket 0 register block in Control Phase
	static const uint8_t BlockSelectTxBuf = (0x02 << 3); //< Socket 0 Tx buffer address block
	static const uint8_t BlockSelectRxBuf = (0x03 << 3); //< Socket 0 Rx buffer address block
	 ...
	 */
	void wizchip_read_buf(uint8_t block, uint16_t address, uint8_t* pBuf,
			uint16_t len);
	 ...
	 * @param len Data length
	 */
	void wizchip_write_buf(uint8_t block, uint16_t address, const uint8_t* pBuf,
			uint16_t len);
